...
## 第二批四条 recording（2026-08-08）

新增运行 `20260612_144154`、`20260612_144214`、`20260722_145020` 和
`20260722_160121`，每条分别使用 `root-assist none` 与 `xy`。8 项均正常运行到
source 结尾，未跌倒、无非有限状态、无 deadline rebase、无 hand overrun，
RTF 均约为 1。`20260722_145020 + xy` 首次运行发生一次 hand overrun，已由通过
全部质量门的重跑结果覆盖。

| recording | assist | body RMSE (rad) | root XY RMS (m) | root XYZ RMS (m) | yaw RMS (deg) |
|---|---:|---:|---:|---:|---:|
| `20260612_144154` | none | 0.08384 | 0.09721 | 0.09765 | 1.839 |
|  | xy | 0.08331 | 0.00000 | 0.00874 | 1.944 |
| `20260612_144214` | none | 0.09714 | 0.06193 | 0.06236 | 2.950 |
|  | xy | 0.08942 | 0.00000 | 0.00936 | 4.602 |
| `20260722_145020` | none | 0.07840 | 0.12097 | 0.12110 | 1.821 |
|  | xy | 0.07697 | 0.00000 | 0.00519 | 1.103 |
| `20260722_160121` | none | 0.07994 | 0.03355 | 0.03373 | 0.926 |
|  | xy | 0.07802 | 0.00000 | 0.00342 | 0.947 |

客观任务物体终态：

- `144154`：none/xy 中垃圾桶踏板和盖子都基本停在初始位置；原 recording 末态
  踏板约为 `-0.11602`、盖子约为 `0.67292`。
- `144214`：none 中踏板/盖终态为 `-0.20895/1.21169`，很接近原 recording 的
  `-0.21122/1.22487`；xy 中只有 `-0.05223/0.30261`。因此这条样本里 XY oracle
  并没有改善任务物体终态。
- `145020`：none 中罐体几乎未离开初始位置；xy 中最终 xyz 与原 recording
  终点的分量误差约为 `(-0.00935, +0.00364, -0.00351) m`，但姿态仍有差异。
- `160121`：none 中罐体基本停在初始位置；xy 中最终 xyz 与原 recording 终点
  的分量误差约为 `(+0.00623, +0.00722, -0.00153) m`，四元数也较接近。

上述仍是物理终态而不是自动任务成功标签；应结合 replay 检查实际接触过程。
