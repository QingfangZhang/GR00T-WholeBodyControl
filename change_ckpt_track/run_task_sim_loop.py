#!/usr/bin/env python3
"""Run the recorded task scene for the qpos-track SONIC comparison.
...
MuJoCo qpos/qvel values at the launcher's selected policy boundary, displays
the live simulation, and writes a replay-compatible CSV under
...
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "change_ckpt_track" / "data"
...
class HandStatePublisher:
    """Publish immutable 200 Hz hand snapshots while MuJoCo releases the GIL."""
    ...
    def __init__(self, env: "TaskSnapshotEnv") -> None:
        ...
        self.thread = threading.Thread(
            target=self._run,
            name="change-ckpt-track-hand-state-publisher",
            daemon=True,
        )
        self.thread.start()
    ...
    def submit(
        ...
        try:
            ...
            # Preserve every 200 Hz state rather than silently dropping one,
...
class TaskSnapshotEnv(DefaultEnv):
    ...
    def prepare_obs(self):
        obs: dict[str, np.ndarray | float] = {
            ...
            # views are sufficient and avoid four allocations per 200 Hz tick.
    ...
    def command_snapshot(self) -> dict[str, np.ndarray]:
        ...
        # at 200 Hz: a callback can otherwise be descheduled while owning the
        # lock and stall MuJoCo for tens of milliseconds.
...
class TaskSimulator:
    ...
    def _apply_root_assist(
        ...
        *,
        ...
        # The floating base was moved discontinuously.  Discard the stale
        # solver warm start and refresh kinematics, contacts and sensors before
        # CSV capture and the next physics/policy-observation tick.
    ...
    def root_assist_summary(self) -> dict[str, object]:
        ...
        return {
            ...
            "source_clock": (
                "original recording rows at the 200 Hz simulator source clock; "
                "independent of canonical/recorded encoder slot lags"
            ),
            "position_update": (
                "hard overwrite after each 200 Hz physics tick and before CSV "
                "capture/the next policy observation"
            ),
            "velocity_update": (
        ...
        try:
            while self._viewer_running():
                ...
                if next_deadline < time.monotonic() - 0.25:
                    # Normal 5--20 ms scheduler/storage jitter is recoverable
                    # because mean tick cost is below 5 ms; do not permanently
                    # shift physics behind the wall-clock reference.  Only
                    # abandon catch-up after a genuinely long UI/OS stall, which
                    # is also recorded as an invalid real-time experiment.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # Match the repository's normal run_sim_loop (200 Hz).  The recording was
    # logged at 400 Hz, so two source rows are consumed per control tick.
    ...
    parser.add_argument(
        ...
        default="none",
        help=(
            "hard-align robot root position/linear velocity to the original "
    ...
    try:
        ...
        if metadata_path is not None:
            ...
            write_metadata(
                metadata_path,
                {
                    "experiment_kind": "qpos_track_in_recorded_task_scene",
                    ...
                    "source_reference_columns_note": (
                        ...
                        "new checkpoint's local encoder input. The actual body reference "
                        "is the qpos-derived 50 Hz stream saved by the track publisher. "
                        + (
                            "Source root qpos/qvel are also hard-applied from the "
                            "original 400 Hz recording timeline by the explicitly "
