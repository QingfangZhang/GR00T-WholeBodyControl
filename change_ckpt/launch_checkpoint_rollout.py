#!/usr/bin/env python3
"""Preflight and launch the CSV-reference SONIC sim2sim experiment.

This file deliberately lives outside the original simulator/deployment sources.
It launches the copies in ``change_ckpt`` and the existing deploy executable,
while keeping every generated artifact under ``change_ckpt/data`` by default.
...
Run only the C++ deployment process (useful with three terminals)::
    ...
    python change_ckpt/launch_checkpoint_rollout.py deploy \
        --checkpoint low_latency
...
SIM_TIMING_PROFILES: dict[int, tuple[float, float]] = {
    200: (0.005, 0.001),
    400: (0.0025, 0.0005),
}
...
DEPLOY_READY_MESSAGES = (
    ...
    "initialized encoder observations",
)
...
def _resolve_sim_timing(args: argparse.Namespace) -> None:
    """Resolve the selected control-rate profile into concrete dt values.

    ``control_dt`` and ``physics_dt`` use ``None`` as their parser defaults so
    we can distinguish a profile-derived value from an explicit, inconsistent
    override.  An explicit physics step is still allowed when it divides the
    selected control period exactly; the usual 200/400 Hz paths use the tested
    profile defaults above.
    """

    frequency = int(getattr(args, "sim_frequency", 200))
    try:
        profile_control_dt, profile_physics_dt = SIM_TIMING_PROFILES[frequency]
    except KeyError as exc:
        supported = ", ".join(str(value) for value in SIM_TIMING_PROFILES)
        raise PreflightError(
            f"Unsupported --sim-frequency {frequency}; choose one of {supported} Hz"
        ) from exc

    control_dt = getattr(args, "control_dt", None)
    if control_dt is None:
        control_dt = profile_control_dt
    elif not math.isclose(
        float(control_dt), profile_control_dt, rel_tol=0.0, abs_tol=1e-12
    ):
        raise PreflightError(
            f"--sim-frequency {frequency} requires "
            f"--control-dt {profile_control_dt:g}, got {float(control_dt):g}"
        )

    physics_dt = getattr(args, "physics_dt", None)
    if physics_dt is None:
        physics_dt = profile_physics_dt

    args.sim_frequency = frequency
    args.control_dt = float(control_dt)
    args.physics_dt = float(physics_dt)


def _selected_sim_frequency(args: argparse.Namespace) -> int:
    """Return the selected control rate while preserving old test namespaces."""

    frequency = getattr(args, "sim_frequency", None)
    if frequency is not None:
        return int(frequency)
    control_dt = getattr(args, "control_dt", None)
    if control_dt is None:
        return 200
    return int(round(1.0 / float(control_dt)))
...
def resolve_model_files(args: argparse.Namespace) -> ModelFiles:
    ...
    if checkpoint == "low_latency":
        default_encoder = CHANGE_ROOT / "models/low_latency/model_encoder.onnx"
    ...
    obs_config = Path(args.obs_config).expanduser() if args.obs_config else default_config
    return ModelFiles(
        checkpoint,
        _absolute(encoder),
        _absolute(decoder),
...
def _validate_recording(recording: Path) -> dict[str, Any]:
    ...
    required = {
        ...
        "reference_motion[0]": lambda name: name == "reference_motion[0]",
...
def run_preflight(args: argparse.Namespace, *, include_adapters: bool = True) -> dict[str, Any]:
    ...
    report["runtime"] = {
        ...
        "timing": {
            "sim_control_rate_hz": args.sim_frequency,
...
def _new_run_dir(args: argparse.Namespace, checkpoint: str) -> Path:
    ...
    assist = getattr(args, "root_assist", "none")
    ...
    result = root / f"{recording_name}_{suffix}"
...
def _write_manifest(
    ...
    _resolve_sim_timing(args)
    physics_substeps = int(round(args.control_dt / args.physics_dt))
    source_rows_per_control = int(round(args.control_dt / args.source_dt))
    payload = {
        ...
        "initialization": {
            ...
            "sim_frequency_profile_hz": args.sim_frequency,
            "sim_control_dt_s": args.control_dt,
            "sim_control_rate_hz": 1.0 / args.control_dt,
            "physics_dt_s": args.physics_dt,
            "physics_rate_hz": 1.0 / args.physics_dt,
            "physics_substeps_per_control": physics_substeps,
            "source_row_dt_s": args.source_dt,
            "source_row_rate_hz": 1.0 / args.source_dt,
            "source_rows_per_control": source_rows_per_control,
            "reference_rate_hz": args.reference_rate,
    ...
    with logfile.open("w", encoding="utf-8", buffering=1) as stream:
        for line in process.stdout:
            ...
            if label == "deploy" and any(message in lower for message in FORBIDDEN_DEPLOY_MESSAGES):
                ...
                fatal_event.set()
            if ready_event and any(message in lower for message in DEPLOY_READY_MESSAGES):
                ready_event.set()
...
def _run_all(args: argparse.Namespace, model: ModelFiles, report: dict[str, Any], run_dir: Path) -> int:
    ...
    # scratch storage, then copy the complete artifacts into change_ckpt/data
    # after every process has stopped.
    deploy_spool: tempfile.TemporaryDirectory[str] | None = None
    deploy_output_root = run_dir
    if not args.dry_run:
        deploy_spool = tempfile.TemporaryDirectory(prefix="change_ckpt_deploy_logs_")
...
def _add_launch_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--sim-frequency",
        type=int,
        choices=tuple(SIM_TIMING_PROFILES),
        default=200,
        help=(
            "MuJoCo control/DDS rate profile: 200 (default; 5 ms control, "
            "1 ms physics) or 400 (2.5 ms control, 0.5 ms physics)"
        ),
    )
    parser.add_argument(
        "--control-dt",
        type=float,
        default=None,
        help="advanced override; must equal the selected --sim-frequency period",
    )
    ...
    parser.add_argument(
        ...
        default="none",
        help=(
            "oracle source-root hard alignment: none (default), xy "
            "(recommended diagnostic), or xyz"
        ),
    )
    parser.add_argument(
        "--physics-dt",
        ...
        default=None,
        help=(
            "MuJoCo integration step; profile default is 1 ms at 200 Hz and "
            "0.5 ms at 400 Hz"
        ),
    )
    ...
    parser.add_argument(
        ...
        action=bool_action,
        default=True,
        help="Restore the recording's initial relative yaw at publisher tick 1",
    )
    parser.add_argument(
        "--base-sample",
