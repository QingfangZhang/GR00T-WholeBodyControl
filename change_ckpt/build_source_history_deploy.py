#!/usr/bin/env python3
"""Build the isolated source-history deploy without editing NVIDIA sources.

The configured ``gear_sonic_deploy/build`` tree already contains the exact
CUDA, TensorRT, ONNX Runtime and Unitree compile/link settings for this host.
This script reuses its compile database and link command, replacing only the
official main object with the wrapper under ``change_ckpt``.  The official
source, object and release binary are hashed before/after and never replaced.
"""
...
OFFICIAL_BINARY = (
    DEPLOY_ROOT / "target/release/g1_deploy_onnx_ref"
).resolve()
WRAPPER_SOURCE = (
    REPO_ROOT
    / "change_ckpt/source_history_deploy/g1_deploy_onnx_ref_source_history.cpp"
).resolve()
PRIVATE_BUILD_ROOT = (REPO_ROOT / "change_ckpt/build/source_history_deploy").resolve()
OUTPUT_BINARY = (
    REPO_ROOT / "change_ckpt/bin/g1_deploy_onnx_ref_source_history"
).resolve()
LINK_FILE = (
    BUILD_ROOT
...
def _load_compile_entry() -> dict[str, Any]:
    if not COMPILE_DATABASE.is_file():
        raise BuildError(
            ...
            "Configure/build gear_sonic_deploy once before building the opt-in wrapper."
        )
...
def _entry_argv(entry: dict[str, Any]) -> list[str]:
    ...
    raise BuildError("compile database entry has neither arguments nor command")
...
def _replace_compile_paths(
    ...
    try:
        ...
        compile_index = result.index("-c") + 1
    except ValueError as exc:
        raise BuildError("unexpected compile command: missing -o or -c") from exc
    if Path(result[compile_index]).resolve() != OFFICIAL_SOURCE:
        raise BuildError(
            f"compile command source changed: {result[compile_index]}"
        )
    result[output_index] = str(output)
    result[compile_index] = str(source)
    return result


def _link_argv(custom_object: Path, temporary_output: Path) -> tuple[list[str], Path]:
    ...
    official_object_suffix = (
        "CMakeFiles/g1_deploy_onnx_ref.dir/src/g1_deploy_onnx_ref.cpp.o"
    )
    object_matches = [
        index
        for index, token in enumerate(argv)
        if token.endswith(official_object_suffix)
    ]
    if len(object_matches) != 1:
        raise BuildError(
            "could not identify exactly one official main object in link.txt"
        )
    argv[object_matches[0]] = str(custom_object)
    try:
        output_index = argv.index("-o") + 1
    except ValueError as exc:
        raise BuildError("unexpected link command: missing -o") from exc
    argv[output_index] = str(temporary_output)
    # CMake executes link.txt from the target's binary directory, not from the
    # nested CMakeFiles/<target>.dir directory that stores link.txt itself.
